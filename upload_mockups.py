"""
NasriTools - Upload Mockup Images to Etsy Listings
Adds laptop mockup as image #2 for each of the 10 top products
Run: python upload_mockups.py
"""
...
DONE_FILE   = Path(os.path.expanduser("~")) / "etsy_mockups_uploaded.json"
...
def upload_image(token, listing_id, img_path, rank=2):
    """Upload an image to an Etsy listing at given rank position."""
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  NasriTools - Upload Mockups to Etsy  [{total} products]")
    ...
    for i, slug in enumerate(PRODUCTS, 1):
        ...
        mockup_path = MOCKUPS_DIR / f"{slug}_mockup.jpg"
        if not mockup_path.exists():
            print(f"[{i:02d}/{total}] SKIP (mockup not found): {mockup_path.name}")
            continue

        print(f"[{i:02d}/{total}] {slug}  (listing {lid})")

        r = upload_image(token, lid, mockup_path, rank=2)
        time.sleep(1)
    ...
    print(f"\n{'='*60}")
    print(f"  Done: {ok}/{total} mockups uploaded to Etsy")
    print(f"{'='*60}\n")
